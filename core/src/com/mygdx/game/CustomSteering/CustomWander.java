package com.mygdx.game.CustomSteering;
...
public class CustomWander<T extends Vector<T>> extends Face<T> {
    ...
    protected float wanderOffset = 20.0f;

    /** The radius of the wander circle */
    protected float wanderRadius = 10.0f;

    /** The maximum rate at which the wander orientation can change */
    protected float wanderRate = 0.001f;

    /** The current orientation of the wander target */
    protected float wanderOrientation = 0.0f;
    ...
    protected SteeringAcceleration<T> calculateRealSteering (SteeringAcceleration<T> steering) {
        ...
        T facingVector = owner.newVector();
        owner.angleToVector(facingVector, owner.getOrientation());

        //scale the facing vector so we can get the circle center
        facingVector = facingVector.nor();
        facingVector.scl(wanderOffset);
        wanderCenter = wanderCenter.add(facingVector);

        T displacementForce = owner.getLinearVelocity().nor();
        displacementForce.scl(wanderRadius);

        owner.angleToVector(displacementForce, wanderOrientation);

        wanderOrientation += ((Math.random() * wanderRate) - (wanderRate* .5) * delta);

        T wanderForce;
        wanderForce = wanderCenter.add(displacementForce);

        steering.linear.set(wanderForce.nor().scl(getActualLimiter().getMaxLinearAcceleration()));
        steering.angular = 0;
        return steering;
    }
